...
Since 2011, the level of financial inclusion has increased. As can be seen in the following graph, the distribution of the account ownership indicator across 126 economies has shifted from the lower to the upper range. Moreover, the distribution as of 2017 shows a multimodal behavior which is basically due to the type of countries included in the sample: developed and developing. 
...
The gap in the financial services adoption is also very clear at State level, and is not only observed in the number of accounts, but in the number of branches as well. When comparing the States, Nuevo León and Mexico City are the ones with high number of accounts and branches per 10,000 inhabitants, whereas Chiapas and Oaxaca are the ones with the lowest numbers. Additionally, there's a clear difference between the North and South regions of the country.    
...
p5<-CNBV_Average_Balance %>% ggplot(aes(x = Year, y = Amount/1000,group=Variable)) + 
  ...
  theme_minimal()+
  theme(legend.position="bottom")+
  labs(subtitle = "Assets and Liabilities Balance.",y="Billions of US dollars")+
...
p6<-CNBV_Average_Indicators %>% ggplot(aes(x = Year, y = Indicator,group=Variable, color=Variable)) + 
  ...
  theme(legend.position="bottom")+
  labs(subtitle = "Credit risk of the overall portfolio.", caption="CNBV: Balance sheet historical series.",y="Percentage (%)", caption="  ")+
  theme(axis.text.x = element_text(color = "grey20", size = 10, angle = 45, hjust = .5, vjust = .5, face = "plain"),
        axis.text.y = element_text(color = "grey20", size = 10, angle = 0, hjust = 1, vjust = 0, face = "plain"),  
        axis.title.x = element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = 0, face = "plain"),
        axis.title.y = element_text(color = "grey20", size = 12, angle = 90, hjust = .5, vjust = .5, face = "plain"),
...
grid.arrange(p5, p6,ncol = 2, nrow = 1, top=textGrob("The banking industry is growing and the credit risk is improving.",gp=gpar(fontsize=20,fontface='bold'),x=0,hjust = 0))

  

```

Moreover, 70% of the deposits are hold by five banks, whereas the remaining 30% is hold by more than 40, which indicates that the industry follows an oligopoly structure. Besides, these five banks hold only 53% of the branches in the country, which might be because of the geographical distribution of the customers. 
...
MEX_Branchs_Bank<-Branchs_bank %>%
  ...
  group_by(.dots=c('Group')) %>%
  summarise(`Branches`=sum(Num_Sucursales)) %>%
  mutate(Branches_p=round(round(100*Branches/Total_branches,1),0)) %>%
...
MEX_map_mun_cred %>% ggplot()+
  ...
  labs(x=NULL, y=NULL, title="The commercial credit service accounts are highly concentrated.",subtitle="In 90% of the 2,458 municipalities in Mexico, the concentration index (HHI) is above 2,500 points, whereas in only 7 is below 1,500.",caption="CNBV: Balance sheet historical series.")+
  theme(axis.text = element_blank(), axis.title = element_blank(), axis.ticks = element_blank(),legend.position = "bottom",plot.subtitle =element_text(color = "grey20", size = 13, angle = 0, hjust = 0, vjust = .5, face = "plain"),
        plot.title =element_text(color = "grey20", size = 19, angle = 0, hjust = 0, vjust = .5, face = "bold"))
